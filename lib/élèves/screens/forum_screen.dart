...
class _ForumScreenState extends State<ForumScreen> {
  ...
  void _addComment(_Post post, String text) {
    if (!AuthService.requireLogin(context, action: 'répondre à une question')) return;
            ...
            child: Container(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Text(post.title, style: TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                  const SizedBox(height: 8),
                  Text('${post.comments.length} réponse${post.comments.length > 1 ? 's' : ''}', style: TextStyle(color: AppColors.textSecondary)),
                  const SizedBox(height: 14),
                  if (post.comments.isEmpty)
                    Padding(
                      padding: const EdgeInsets.symmetric(vertical: 20),
                      child: Text('Soyez le premier à répondre à cette question.', style: TextStyle(color: AppColors.textSecondary)),
                    )
                  else
                    Column(children: post.comments.map((comment) => Container(
                      margin: const EdgeInsets.only(bottom: 12),
                      padding: const EdgeInsets.all(14),
                      decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(16)),
                      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween, children: [
                          Text(comment.author, style: TextStyle(fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                          Row(children: [
                            Icon(Icons.thumb_up_rounded, size: 16, color: AppColors.textTertiary),
                            const SizedBox(width: 6),
                            Text('${comment.likes}', style: TextStyle(fontSize: 12, color: AppColors.textSecondary)),
                          ]),
                        ]),
                        const SizedBox(height: 8),
                        Text(comment.text, style: TextStyle(color: AppColors.textSecondary, height: 1.5)),
                        const SizedBox(height: 8),
                        Row(children: [
                          IconButton(
                            onPressed: () {
                            ...
                            icon: Icon(Icons.thumb_up_alt_outlined, color: AppColors.brand),
                          ),
                          Text(comment.timeAgo, style: TextStyle(color: AppColors.textTertiary, fontSize: 12)),
                        ]),
                      ]),
                    )).toList()),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton: FloatingActionButton.extended(
        onPressed: () {
          if (!AuthService.requireLogin(context, action: 'poster une question dans le forum')) return;
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => ForumAskQuestionScreen(
                subjects: subjects,
                onPostCreated: (title, content, subject) {
                  setState(() {
                    posts.insert(0, _Post(
                      posts.length + 1,
                      AuthService.currentUser?.fullName ?? 'Utilisateur',
                      AuthService.currentUser?.fullName.characters.first.toUpperCase() ?? 'U',
                      title,
                      content,
                      subject,
                      0,
          ...
          TextField(
            ...
            onChanged: (v) {
              if (!AuthService.isLoggedIn) {
                AuthService.requireLogin(context, action: 'rechercher dans le forum');
                return;
              }
              setState(() => _searchQuery = v);
            },
            decoration: InputDecoration(
              hintText: AuthService.isLoggedIn ? 'Rechercher...' : 'Connectez-vous pour rechercher',
        ...
        Expanded(child: f.isEmpty
          ...
          : ListView.builder(padding: const EdgeInsets.symmetric(horizontal: 16), itemCount: f.length,
              itemBuilder: (c, i) => _PostCard(
                    post: f[i],
                    onLike: () {
                      if (!AuthService.requireLogin(context, action: 'aimer une question')) return;
      ...
      child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
        Row(children: [
          ...
          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
            ...
            Row(children: [
              Text('@${post.author.split(' ').first.toLowerCase()}', style: TextStyle(fontSize: 12, color: AppColors.textTertiary)),
              const SizedBox(width: 8),
        ...
        Row(children: [
          IconButton(
            onPressed: onLike,
            icon: Icon(Icons.thumb_up_alt_rounded, size: 20, color: AppColors.textTertiary),
          ),
          Text('${post.likes}', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
          const SizedBox(width: 24),
          IconButton(
            onPressed: onTap,
            icon: Icon(Icons.chat_bubble_outline_rounded, size: 20, color: AppColors.textTertiary),
          ),
          Text('${post.replies}', style: TextStyle(fontSize: 13, color: AppColors.textSecondary)),
        ]),
      ]),
    ));
  }
...
class _Comment {
  ...
  _Comment({required this.author, required this.text, required this.timeAgo, required this.likes});
}
...
class _Post {
  ...
  _Post(this.id, this.author, this.avatar, this.title, this.content, this.subject, this.likes, this.replies, this.timeAgo, this.tags, this.solved, this.comments);
}
            ...
            child: SingleChildScrollView(
              ...
              child: Column(
                ...
                children: [
                  ...
                  Container(
                    ...
                    child: Column(
                      ...
                      children: [
                        Row(children: [
                          CircleAvatar(radius: 20, backgroundColor: AppColors.brand,
                            child: Text(widget.post.avatar, style: const TextStyle(color: Colors.white, fontWeight: FontWeight.w700, fontSize: 13))),
                          const SizedBox(width: 12),
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Row(children: [
                              Text(widget.post.author, style: const TextStyle(fontWeight: FontWeight.w700, color: AppColors.textPrimary, fontSize: 14)),
                              const SizedBox(width: 8),
                              Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                decoration: BoxDecoration(color: AppColors.brandLight, borderRadius: BorderRadius.circular(6)),
                                child: Text(widget.post.subject, style: const TextStyle(fontSize: 11, color: AppColors.brand, fontWeight: FontWeight.w500))),
                            ]),
                            Row(children: [
                              Text('@${widget.post.author.split(' ').first.toLowerCase()}', style: const TextStyle(fontSize: 12, color: AppColors.textTertiary)),
                              const SizedBox(width: 8),
                              const Text('·', style: TextStyle(fontSize: 12, color: AppColors.textTertiary)),
                              const SizedBox(width: 8),
                              Text(widget.post.timeAgo, style: const TextStyle(fontSize: 12, color: AppColors.textTertiary)),
                            ])
                          ])),
                        ]),
                        const SizedBox(height: 16),
                        Text(widget.post.title, style: const TextStyle(fontSize: 18, fontWeight: FontWeight.w700, color: AppColors.textPrimary)),
                        const SizedBox(height: 8),
                        Text(widget.post.content, style: const TextStyle(fontSize: 15, color: AppColors.textSecondary, height: 1.5)),
                        const SizedBox(height: 12),
                        Wrap(spacing: 6, children: widget.post.tags.map((t) => Container(padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 4),
                          decoration: BoxDecoration(color: AppColors.background, borderRadius: BorderRadius.circular(6)),
                          child: Row(mainAxisSize: MainAxisSize.min, children: [const Icon(Icons.label_rounded, size: 12, color: AppColors.textTertiary), const SizedBox(width: 4),
                            Text(t, style: const TextStyle(fontSize: 12, color: AppColors.textSecondary))]))).toList()),
                        const SizedBox(height: 16),
                        ...
                        Row(children: [
                          IconButton(
                            ...
                            icon: const Icon(Icons.thumb_up_alt_rounded, size: 20, color: AppColors.textTertiary),
                          ),
                          Text('${widget.post.likes}', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                          const SizedBox(width: 24),
                          const Icon(Icons.chat_bubble_outline_rounded, size: 20, color: AppColors.textTertiary),
                          const SizedBox(width: 4),
                          Text('${widget.post.replies}', style: const TextStyle(fontSize: 13, color: AppColors.textSecondary)),
                        ]),
                      ],
                    ),
                  ),
                  ...
                  const Text('Commentaires', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
                  const SizedBox(height: 16),
                  ...
                  if (widget.post.comments.isEmpty)
                    const Padding(
                      ...
                      child: Center(child: Text('Aucun commentaire pour le moment. Soyez le premier !', style: TextStyle(color: AppColors.textSecondary))),
                    )
                  else
                    ...widget.post.comments.asMap().entries.map((entry) {
                      ...
                      return Padding(
                        ...
                        child: Row(crossAxisAlignment: CrossAxisAlignment.start, children: [
                          CircleAvatar(radius: 16, backgroundColor: AppColors.brandLight,
                            child: Text(comment.author.characters.first.toUpperCase(), style: const TextStyle(color: AppColors.brand, fontWeight: FontWeight.w700, fontSize: 13)),
                          ),
                          ...
                          Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                            Container(
                              ...
                              child: Column(
                                crossAxisAlignment: CrossAxisAlignment.start,
                                children: [
                                  Text(comment.author, style: const TextStyle(fontWeight: FontWeight.w700, color: AppColors.textPrimary, fontSize: 13)),
                                  const SizedBox(height: 4),
                                  Text(comment.text, style: const TextStyle(color: AppColors.textSecondary, height: 1.4)),
                                ],
                              ),
                            ),
                            const SizedBox(height: 4),
                            Row(children: [
                              const SizedBox(width: 12),
                              Text(comment.timeAgo, style: const TextStyle(color: AppColors.textTertiary, fontSize: 12)),
                              ...
                              GestureDetector(
                                ...
                                child: Text('J\'aime', style: TextStyle(color: comment.likes > 0 ? AppColors.brand : AppColors.textSecondary, fontSize: 12, fontWeight: FontWeight.w600)),
                              ),
                              if (comment.likes > 0) ...[
                                const SizedBox(width: 6),
                                const Icon(Icons.thumb_up_alt_rounded, size: 12, color: AppColors.brand),
                                const SizedBox(width: 2),
                                Text('${comment.likes}', style: const TextStyle(color: AppColors.textSecondary, fontSize: 12)),
                              ]
                            ]),
                          ])),
                        ]),
          ...
          Container(
            ...
            child: Row(children: [
              ...
              IconButton(
                ...
                padding: const EdgeInsets.all(12),
                style: IconButton.styleFrom(backgroundColor: AppColors.brandLight),
              ),
            ]),
          ),
        ],
...
class ForumAskQuestionScreen extends StatefulWidget {
  ...
  final Function(String title, String content, String subject) onPostCreated;
...
class _ForumAskQuestionScreenState extends State<ForumAskQuestionScreen> {
  ...
  void initState() {
    super.initState();
    _subject = widget.subjects.firstWhere((s) => s != 'all', orElse: () => widget.subjects.first);
  }
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            const Text('Titre de la question', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
            const SizedBox(height: 8),
            TextField(
            ...
            const Text('Détails de votre problème', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
            const SizedBox(height: 8),
            TextField(
              controller: _contentController,
              maxLines: 6,
              decoration: InputDecoration(
                hintText: 'Précisez où vous êtes bloqué, ce que vous ne comprenez pas...',
                filled: true,
                fillColor: Colors.white,
                border: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: AppColors.divider)),
                enabledBorder: OutlineInputBorder(borderRadius: BorderRadius.circular(16), borderSide: BorderSide(color: AppColors.divider)),
            ...
            const Text('Matière', style: TextStyle(fontWeight: FontWeight.w700, fontSize: 16)),
            const SizedBox(height: 8),
            Container(
              ...
              child: DropdownButtonHideUnderline(
                child: DropdownButton<String>(
                  value: _subject,
                  isExpanded: true,
                  items: widget.subjects.where((s) => s != 'all').map((s) => DropdownMenuItem(value: s, child: Text(s))).toList(),
                  onChanged: (value) {
            ...
            ElevatedButton(
              onPressed: () {
                ...
                widget.onPostCreated(title, content, _subject);
              ...
              child: const Text('Publier la question', style: TextStyle(fontSize: 16, fontWeight: FontWeight.w700)),
            ),
          ],
        ),
      ),
